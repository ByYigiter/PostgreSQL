...
-- in ile farkı headerlar existsde hepsi aynı olmalı in komutunda tamammen farklı olabilir
CREATE TABLE mart
(   
urun_id int,    
...
-- in ile yapımı
select urun_id,musteri_isim from mart where urun_id in(select urun_id from nisan where urun_id=nisan.urun_id );

--Her iki ayda birden satılan ürünlerin URUN_ISIM'lerini ve bu ürünleri
--NİSAN ayında satın alan MUSTERI_ISIM'lerini listeleyen bir sorgu yazınız.

select musteri_isim, urun_isim from nisan where exists (select urun_id from mart where mart.urun_isim=nisan.urun_isim);

-- update set kullanımı
