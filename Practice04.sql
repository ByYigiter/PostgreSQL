...
select round(avg(fiyat),3)from arac;
...
select * from meslekler offset 2 limit 5;-- offset komutu atlama komutu skip gibi 2 tane atla demek
...
select filmler.category,filmler.film_name,aktorler.actor_name
from filmler inner join aktorler
on filmler.film_id=aktorler.film_id;

select film_name,category,actor_name -- ana liste  Tüm film_name'leri olark listelemek için left yada right olarak yazmallıyız
...
select qa_dersler.ders_ismi from qa_dersler
left join developer_dersler on developer_dersler.ders_ismi=qa_dersler.ders_ismi
union
select developer_dersler.ders_ismi from qa_dersler
right join developer_dersler on developer_dersler.ders_ismi=qa_dersler.ders_ismi
